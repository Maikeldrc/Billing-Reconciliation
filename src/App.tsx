...
import { Claim, ClaimStatus, ClaimClassification, ErrorCategory, Payment, Note, AuditLog, Provider, Payer, User, Setting, UserRole, FeeSchedule } from "./types";
...
export default function App() {
  ...
  const [newCharge, setNewCharge] = useState<number>(150);
  const [newProviderId, setNewProviderId] = useState("PROV_01");
  const [newPayerId, setNewPayerId] = useState("PAY_01");
  const [newServiceType, setNewServiceType] = useState("RPM");
  const [newCpt, setNewCpt] = useState("99454");

  // Fee Schedule management UI states
  const [settingsTab, setSettingsTab] = useState<"fee-schedules" | "contract-rules" | "sheets-sync">("fee-schedules");
  ...
  // Fetch all initial server values on mount
  useEffect(() => {
    fetchAllData();
  }, []);
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const matched = usrData.find((u: User) => u.user_id === currentUser.user_id);
      if (matched) {
  ...
  const handleSyncWithGoogleSheets = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(`Fallo de sincronización: ${data.error}`);
      }
    } catch (err: any) {
  ...
  const handleCreateClaimManually = async (e: React.FormEvent) => {
    ...
    // If Claim ID is empty or not provided, pass "AUTO_GENERATE" so server automatically generates it
    const claimIdToSend = newClaimId.trim() || "AUTO_GENERATE";

    const rawClaim: Partial<Claim> = {
      claim_id: claimIdToSend,
      ...
      billed_charge: Number(newCharge),
      allowed_amount: 0,
      paid_amount: 0,
    ...
    try {
      ...
      alert("¡Claim creado y recalculado exitosamente!");
  ...
  const handleUpdateClaim = async (updates: Partial<Claim>) => {
    ...
    const res = await fetch(`/api/claims/${selectedClaim.claim_id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "x-user-email": currentUser.email
    ...
    if (!res.ok) {
      const errData = await res.json();
      throw new Error(errData.error || errData.details?.join("; ") || "Failed to update claim");
    }
  ...
  const handleApplyBulkAction = async (actionType: string, value: any) => {
    ...
    if (actionType === "status") {
      ...
      alert("¡Nota agregada de forma masiva!");
      setSelectedClaimIds([]);
      await fetchAllData();
      return;
    }
    ...
    try {
      const res = await fetch("/api/claims/bulk-update", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "x-user-email": currentUser.email
      ...
      alert("¡Acción masiva completada con éxito!");
      setSelectedClaimIds([]);
      await fetchAllData();
    } catch (err: any) {
  ...
  const handleImportCSV = async (rows: any[]) => {
  ...
  const handleUpdateSetting = async (key: string, value: string) => {
    ...
    if (res.ok) {
      alert("Ajuste contractual actualizado.");
      await fetchAllData();
    } else {
  ...
  const handleSaveFeeSchedule = async () => {
    if (!fsCptCode.trim()) {
      alert("Por favor ingresa un código CPT.");
      return;
    }
    const payload = {
      cpt_code: fsCptCode,
    ...
    try {
      let res;
      if (editingFs) {
        res = await fetch(`/api/fee-schedules/${editingFs.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload)
        });
      } else {
        res = await fetch("/api/fee-schedules", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload)
        });
      ...
      if (res.ok) {
        ...
        const errData = await res.json();
        alert(`Error: ${errData.error || "No se pudo guardar la tarifa"}`);
      }
    } catch (err: any) {
  ...
  const handleDeleteFeeSchedule = async (id: string) => {
    if (!confirm("¿Está seguro de que desea eliminar esta tarifa del Fee Schedule?")) return;
    try {
  ...
  const handleRecordPhysicianPayout = async (providerId: string, amount: number) => {
    ...
    if (providerClaims.length === 0) {
      alert("No hay saldos pendientes (Ending A/P) por pagar a este médico.");
      return;
    }
    ...
    for (const claim of providerClaims) {
      ...
      await fetch(`/api/claims/${claim.claim_id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          "x-user-email": currentUser.email
    ...
    alert(`Se registraron distribuciones de pago por un total de $${amount} entre los claims del médico.`);
    await fetchAllData();
  };
  ...
  // Calculate high quality aggregates for the dashboard KPIs
  const totalBilledCharges = filteredClaims.reduce((acc, c) => acc + c.billed_charge, 0);
  const totalInsuranceAdjustments = filteredClaims.reduce((acc, c) => acc + c.insurance_adjustment, 0);
  const totalDenialsWriteoffs = filteredClaims.reduce((acc, c) => acc + c.denied_amount + c.write_off_amount + c.uncollectible_amount, 0);
  const totalNetCollectible = filteredClaims.reduce((acc, c) => acc + c.net_collectible_revenue, 0);
  const totalCollections = filteredClaims.reduce((acc, c) => acc + c.total_collections, 0);
  const iteraCollections = filteredClaims.reduce((acc, c) => acc + c.itera_direct_collection, 0);
  const providerCollections = filteredClaims.reduce((acc, c) => acc + c.provider_direct_collection, 0);
  const totalARBalance = filteredClaims.reduce((acc, c) => acc + c.ar_balance, 0);
  const iteraARBalance = filteredClaims.reduce((acc, c) => acc + c.itera_ar, 0);
  const providerARBalance = filteredClaims.reduce((acc, c) => acc + c.provider_ar, 0);
  const totalPayableToPhysician = filteredClaims.reduce((acc, c) => acc + c.account_payable_to_physician, 0);
  const totalPaymentToPhysician = filteredClaims.reduce((acc, c) => acc + c.payment_to_physician, 0);
  const totalEndingAP = filteredClaims.reduce((acc, c) => acc + c.ending_ap_to_physician, 0);
  const totalNetIteraRevenue = filteredClaims.reduce((acc, c) => acc + c.net_itera_revenue, 0);
  const totalNetProviderRevenue = filteredClaims.reduce((acc, c) => acc + c.net_provider_revenue, 0);

  // Status counters
  const countPending = filteredClaims.filter(c => c.claim_status === ClaimStatus.Pending).length;
  const countPaid = filteredClaims.filter(c => c.claim_status === ClaimStatus.Paid).length;
  const countPartiallyPaid = filteredClaims.filter(c => c.claim_status === ClaimStatus.PartiallyPaid).length;
  const countDenied = filteredClaims.filter(c => c.claim_status === ClaimStatus.Denied).length;
  const countRejected = filteredClaims.filter(c => c.claim_status === ClaimStatus.Rejected).length;
  const countBlocked = filteredClaims.filter(c => c.claim_status === ClaimStatus.BlockedByError).length;
  const countReadyToRebill = filteredClaims.filter(c => c.claim_status === ClaimStatus.ReadyToRebill).length;
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 pb-32">
          ...
          {currentView === "dashboard" && (
            <div className="space-y-6">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h2 className="text-xl md:text-2xl font-bold text-slate-900 font-display tracking-tight">
                    Dashboard de Conciliación Financiera
                  </h2>
                  <p className="text-xs text-slate-500 mt-1">
                    Control de saldos, payouts médicos y métricas de reclamaciones de ITERA HEALTH
                  </p>
                </div>
              ...
              {/* KPI Section A: Billing & Adjustments */}
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono mb-2">Totales de Facturación & Castigos</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                  <KPICard title="Total Billed Charges" value={formatUSD(totalBilledCharges)} icon={FileSpreadsheet} iconColorClass="text-blue-500" />
                  <KPICard title="Insurance Adjustments" value={formatUSD(totalInsuranceAdjustments)} icon={Sliders} iconColorClass="text-slate-400" />
                  <KPICard title="Denials & Write-offs" value={formatUSD(totalDenialsWriteoffs)} icon={FileX} iconColorClass="text-rose-500" />
                  <KPICard title="Net Collectible Revenue" value={formatUSD(totalNetCollectible)} icon={CheckCircle2} iconColorClass="text-emerald-500" />
                </div>
              </div>

              {/* KPI Section B: Collections Breakdown */}
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono mb-2">Breakdown de Cobros Recibidos (Collections)</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  <KPICard title="Total Collections Received" value={formatUSD(totalCollections)} icon={Coins} iconColorClass="text-teal-500" />
                  <KPICard title="ITERA Direct Collections" value={formatUSD(iteraCollections)} icon={Database} iconColorClass="text-blue-600" />
                  <KPICard title="Provider Direct Collections" value={formatUSD(providerCollections)} icon={Hospital} iconColorClass="text-sky-600" />
                </div>
              </div>

              {/* KPI Section C: Accounts Receivable (A/R) Balances */}
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono mb-2">Saldos por Cobrar (Accounts Receivable - A/R)</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  <KPICard title="Total A/R Balance" value={formatUSD(totalARBalance)} icon={Coins} iconColorClass="text-amber-500" />
                  <KPICard title="ITERA A/R Balance" value={formatUSD(iteraARBalance)} icon={Database} iconColorClass="text-blue-500" />
                  <KPICard title="Provider A/R Balance" value={formatUSD(providerARBalance)} icon={Hospital} iconColorClass="text-sky-500" />
                </div>
              </div>

              {/* KPI Section D: Physician Payouts & Part Shares */}
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono mb-2">Distribución de Ingresos y Payouts de Médicos</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
                  <KPICard title="AP Payable to Physician" value={formatUSD(totalPayableToPhysician)} icon={Coins} iconColorClass="text-indigo-500" />
                  <KPICard title="Payouts Distributed" value={formatUSD(totalPaymentToPhysician)} icon={CheckCircle2} iconColorClass="text-emerald-600" />
                  <KPICard title="Ending AP Owed to Physician" value={formatUSD(totalEndingAP)} icon={Lock} iconColorClass="text-indigo-600 animate-pulse-slow" />
                  <KPICard title="Net ITERA Revenue (30%)" value={formatUSD(totalNetIteraRevenue)} icon={Database} iconColorClass="text-blue-700" />
                  <KPICard title="Net Provider Revenue (70%)" value={formatUSD(totalNetProviderRevenue)} icon={Hospital} iconColorClass="text-emerald-700" />
                </div>
              </div>

              {/* KPI Section E: Clickable State/Status Counters */}
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono mb-2">Métricas de Auditoría (Haga clic para filtrar la lista)</p>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-7 gap-3">
                  <div onClick={() => handleKPICardClick("status", ClaimStatus.Pending)} className="bg-white border border-slate-200 hover:border-amber-500 cursor-pointer p-4 rounded-xl text-center transition-all">
                    <span className="block text-[10px] text-slate-400 font-mono uppercase">Pending</span>
                    <span className="text-xl font-bold text-slate-800">{countPending}</span>
                  </div>
                  <div onClick={() => handleKPICardClick("status", ClaimStatus.Paid)} className="bg-white border border-slate-200 hover:border-emerald-500 cursor-pointer p-4 rounded-xl text-center transition-all">
                    <span className="block text-[10px] text-slate-400 font-mono uppercase">Paid</span>
                    <span className="text-xl font-bold text-slate-800">{countPaid}</span>
                  </div>
                  <div onClick={() => handleKPICardClick("status", ClaimStatus.PartiallyPaid)} className="bg-white border border-slate-200 hover:border-cyan-500 cursor-pointer p-4 rounded-xl text-center transition-all">
                    <span className="block text-[10px] text-slate-400 font-mono uppercase">Partial Paid</span>
                    <span className="text-xl font-bold text-slate-800">{countPartiallyPaid}</span>
                  </div>
                  <div onClick={() => handleKPICardClick("status", ClaimStatus.Denied)} className="bg-white border border-slate-200 hover:border-rose-500 cursor-pointer p-4 rounded-xl text-center transition-all">
                    <span className="block text-[10px] text-slate-400 font-mono uppercase">Denied</span>
                    <span className="text-xl font-bold text-rose-600">{countDenied}</span>
                  </div>
                  <div onClick={() => handleKPICardClick("status", ClaimStatus.Rejected)} className="bg-white border border-slate-200 hover:border-red-500 cursor-pointer p-4 rounded-xl text-center transition-all">
                    <span className="block text-[10px] text-slate-400 font-mono uppercase">Rejected</span>
                    <span className="text-xl font-bold text-red-600">{countRejected}</span>
                  </div>
                  <div onClick={() => handleKPICardClick("errorFlag", "true")} className="bg-white border border-red-100 hover:border-red-500 hover:bg-rose-50/5 cursor-pointer p-4 rounded-xl text-center transition-all">
                    <span className="block text-[10px] text-rose-500 font-mono uppercase font-bold">Errors</span>
                    <span className="text-xl font-bold text-rose-600 animate-pulse">{countBlocked}</span>
                  </div>
                  <div onClick={() => handleKPICardClick("status", ClaimStatus.ReadyToRebill)} className="bg-white border border-slate-200 hover:border-orange-500 cursor-pointer p-4 rounded-xl text-center transition-all">
                    <span className="block text-[10px] text-slate-400 font-mono uppercase">To Rebill</span>
                    <span className="text-xl font-bold text-slate-800">{countReadyToRebill}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {currentView === "claims" && (
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h2 className="text-xl md:text-2xl font-bold text-slate-900 font-display tracking-tight">
                    Worklist de Reclamaciones (Claims)
                  </h2>
                  <p className="text-xs text-slate-500 mt-1">
                    Audita, clasifica y gestiona los ciclos financieros de cada claim de Digital Care Management
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                        ...
                        return (
                          <tr key={p.provider_id} className="hover:bg-slate-50 font-medium">
                            ...
                            <td className="p-3.5 text-center">
                              <button
                                onClick={() => {
                                  const amtStr = prompt(`Registrar distribución de pago para ${p.provider_name}. Saldo pendiente actual: ${formatUSD(ending)}. Ingresa monto:`);
                                  if (amtStr) {
            ...
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Administra las reglas contractuales, los honorarios FCSO y la sincronización en tiempo real
                  </p>
                </div>
              </div>
              ...
              <div className="flex border-b border-slate-200 gap-1 overflow-x-auto pb-px">
                ...
                >
                  ...
                  <span>Reglas Contractuales (Shares)</span>
                </button>
                <button
                ...
                >
                  ...
                  <span>Integración Google Sheets</span>
                </button>
              </div>
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 bg-slate-950/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-slate-200 overflow-hidden">
            ...
            <form onSubmit={handleCreateClaimManually} className="p-6 space-y-4 text-xs font-sans">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 mb-1">ID de Claim (Vacío = Automático)</label>
                  <input
                    type="text"
                    placeholder="Generado automáticamente"
                    value={newClaimId}
                    onChange={(e) => setNewClaimId(e.target.value)}
                    className="w-full p-2 border border-slate-200 rounded bg-slate-50 font-mono uppercase"
                  />
                </div>
                <div>
                  <label className="block text-slate-500 mb-1">Billed Charge ($) (Opcional - Autocalcula)</label>
                  <input
                    type="number"
                    placeholder="Se calcula según CPT Fee Schedule"
                    value={newCharge || ""}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="MRN-10293 (Opcional)"
                    value={newPatientId}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-slate-500 mb-1">Servicio</label>
                  <select
                    value={newServiceType}
                    onChange={(e) => setNewServiceType(e.target.value)}
                    className="w-full p-2 border border-slate-200 rounded bg-slate-50 text-slate-700 font-mono"
                  >
                ...
                <div>
                  <label className="block text-slate-500 mb-1">CPT Code</label>
                  <input
                    type="text"
                    placeholder="99454"
                    value={newCpt}
